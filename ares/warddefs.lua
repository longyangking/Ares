...
function wards.GetDistance(s, t)
    --print("S1: "..s[1]..", S2: "..s[2].." :: T1: "..t[1]..", T2: "..t[2]);
    return math.sqrt((s[1]-t[1])*(s[1]-t[1]) + (s[2]-t[2])*(s[2]-t[2]));
end

function wards.GetMandatorySpot()
...
function wards.GetWardSpotWhenTowerFall()
	local wardSpot = {};
	for i = 1, #Towers
	do
...
function wards.GetAggressiveSpot()
...
function wards.GetItemWard(bot)
	for i = 0,8 do
...
function wards.IsPingedByHumanPlayer(bot)
...
function wards.GetAvailableSpot(bot)
	local temp = {};
	for _,s in pairs(wards.GetMandatorySpot()) do
		if not wards.CloseToAvailableWard(s) then
			table.insert(temp, s);
		end
	end
	for _,s in pairs(wards.GetWardSpotWhenTowerFall()) do
		if not wards.CloseToAvailableWard(s) then
			table.insert(temp, s);
		end
	end
	if DotaTime() > 5*60 then
		for _,s in pairs(wards.GetAggressiveSpot()) do
			if GetUnitToLocationDistance(bot, s) <= 1200 and not wards.CloseToAvailableWard(s) then
				table.insert(temp, s);
			end
		end
	end
...
function wards.CloseToAvailableWard(wardLoc)
	local WardList = GetUnitList(UNIT_LIST_ALLIED_WARDS);
	for _,ward in pairs(WardList) do
		if wards.IsObserver(ward) and GetUnitToLocationDistance(ward, wardLoc) <= visionRad then
			return true;
		end
	end
	return false;
...
function wards.GetClosestSpot(bot, spots)
	local cDist = 100000;
	local cTarget = nil;
...
function wards.IsObserver(wardUnit)
	return wardUnit:GetUnitName() == "npc_dota_observer_wards";
end

function wards.GetHumanPing()
