...
module("genericutils", package.seeall)
...
function ItemPurchase(ItemsToBuy)
	...
	then
		...
		then
			if (IsItemPurchasedFromSideShop( sNextItem ) and npcBot:DistanceFromSideShop() <= 4000)  --ֻ�������·�̵�Ͻ�ʱ��ǰ����·�̵�
			then
		...
		elseif(npcBot.secretShopMode == true)		--���Ŀ���������̵꣬��������ʹ������Ʒ
		then
			...
			then
				...
				if(courier:DistanceFromSecretShop() <= 250 and ItemCount<9)		--��ʹ�ѵ����̵�
				then
		...
		if(PurchaseResult==PURCHASE_ITEM_SUCCESS)		--�ɹ������ӳ�װ�����Ƴ�����Ʒ
		then
			npcBot.secretShopMode = false;
			npcBot.sideShopMode = false;
			table.remove( ItemsToBuy, 1 )
		end
		if(PurchaseResult==PURCHASE_ITEM_OUT_OF_STOCK)	--��Ʒ�����������۶������Ʒ
		then
			SellSpecifiedItem("item_branches")
			SellSpecifiedItem("item_dust")
			SellSpecifiedItem("item_faerie_fire")
		...
		if(PurchaseResult==PURCHASE_ITEM_INVALID_ITEM_NAME or PurchaseResult==PURCHASE_ITEM_DISALLOWED_ITEM)	--�����ڵ���Ʒ���Ƴ�����Ʒ
		then
			table.remove( ItemsToBuy, 1 )
		end
		if(PurchaseResult==PURCHASE_ITEM_INSUFFICIENT_GOLD )	--���㣨��ʵ�����Ҳ���ٳ��֣���Ϊ�����Ѿ��������ж��˽�Ǯ��
		then
			npcBot.secretShopMode = false;
			npcBot.sideShopMode = false;
		end
		if(PurchaseResult==PURCHASE_ITEM_NOT_AT_SECRET_SHOP)	--���������̵꣬ǰ�������̵�
		then
			npcBot.secretShopMode = true
			npcBot.sideShopMode = false;
		end
		if(PurchaseResult==PURCHASE_ITEM_NOT_AT_SIDE_SHOP)		--���ڱ�·�̵꣨��ʵ�����������֣���Ϊ�ڱ�·�̵����Ʒ���������̵깺��
		then
			npcBot.sideShopMode = true
			npcBot.secretShopMode = false;
		end
		if(PurchaseResult==PURCHASE_ITEM_NOT_AT_HOME_SHOP)		--���ڻ����̵꣨Ҳ������ֵ��������Ϊ���Ӣ�۲��ڼ��У���ô��Ʒ�Ṻ�������ش���
		then
			npcBot.secretShopMode = false;
			npcBot.sideShopMode = false;
		end
